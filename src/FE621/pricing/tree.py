...
class CRRBinomialTree():
    """
    Additive binomial tree pricing model with CRR step sizes
    """
    def __init__(self, r_f: float, sigma: float, s0: float, T: float, n: int):
        self.r_f = r_f
        self.sigma = sigma
        ...
        # step size delta r
        self.d_r = self.sigma * np.sqrt(self.dt)

        # risk neutral probability of up move
        self.p_u = (np.exp(r_f * self.dt) - np.exp(-self.d_r)) / (np.exp(self.d_r) - np.exp(-self.d_r))
...
if __name__ == "__main__":
    ...
    crr_tree = CRRBinomialTree(r, sigma, S0, T, steps)

    # print(bin_tree.stock_tree)

    print(crr_tree.price_option(K, call=True, american=False))
    ...
    print(tree.price_option(K, call=True, american=False)) # at 2 steps, 135 is first tier up
